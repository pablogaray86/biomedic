<?php
/**
 * @version 1.0
 */

require("class.phpmailer.php");
require("class.smtp.php");

// Valores enviados desde el formulario
if ( !isset($_POST["nombre"]) || !isset($_POST["email"]) || !isset($_POST["telefono"]) ) {
    die ("Es necesario completar todos los datos del formulario");
}
$nombre = $_POST["nombre"];
$email = $_POST["email"];
$telefono = $_POST["telefono"];


// Datos de la cuenta de correo utilizada para enviar vía SMTP
$smtpHost = "c1380094.ferozo.com";  // Dominio alternativo brindado en el email de alta 
$smtpUsuario = "contacto@c1380094.ferozo.com";  // Mi cuenta de correo
$smtpClave = "Med1c1na";  // Mi contraseña

// Email donde se enviaran los datos cargados en el formulario de contacto
$emailDestino = "contacto@c1380094.ferozo.com";

$mail = new PHPMailer();
$mail->IsSMTP();
$mail->SMTPAuth = true;
$mail->Port = 587; 
$mail->IsHTML(true); 
$mail->CharSet = "utf-8";

// VALORES A MODIFICAR //
$mail->Host = $smtpHost; 
$mail->Username = $smtpUsuario; 
$mail->Password = $smtpClave;

$mail->From = $email; // Email desde donde envío el correo.
$mail->FromName = $nombre;
$mail->AddAddress($emailDestino); // Esta es la dirección a donde enviamos los datos del formulario
$mail->addBcc("sarevalo@aldeaglobal.com.ar");

$mail->Subject = "Datos de quien descargo el software"; // Este es el titulo del email.
$mensajeHtml = "Nombre: " . nl2br($nombre) . "<br>";
$mensajeHtml .= "Email: " . nl2br($email). "<br>";
$mensajeHtml .= "Telefono: " . nl2br($telefono). "<br>";


$mail->Body = "{$mensajeHtml} <br /><br />Mensaje recibido desde sitio web Biomedic<br />"; // Texto del email en formato HTML
$mail->AltBody = "\n\n Desarrollo www.aldeaglobal.com.ar"; // Texto sin formato HTML
// FIN - VALORES A MODIFICAR //

$mail->SMTPOptions = array(
    'ssl' => array(
        'verify_peer' => false,
        'verify_peer_name' => false,
        'allow_self_signed' => true
    )
);

$estadoEnvio = $mail->Send(); 

if($estadoEnvio){
    header("Content-disposition: attachment; filename=descargas\BiomedicHolterECG-1.19-Setup.exe");
header("Content-type: MIME");
readfile("descargas\BiomedicHolterECG-1.19-Setup.exe");
	exit();
} else {
    header("Content-disposition: attachment; filename=descargas\BiomedicHolterECG-1.19-Setup.exe");
    header("Content-type: MIME");
    readfile("descargas\BiomedicHolterECG-1.19-Setup.exe");
    exit();

}
